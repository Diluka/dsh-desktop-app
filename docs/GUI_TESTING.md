...
> 没有安装 dsh 的机器上，可用 `scripts/fake_dsh_windows/` 里的 `dsh.ps1` + `fake-dsh.js`
...
> `scripts/fake_dsh_windows/run-fake-dsh-test.ps1`（把 `dsh.ps1`/`fake-dsh.js` 与 `DSH-Desktop.exe`
> 放同一目录）。
