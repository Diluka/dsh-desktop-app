...
3. 发行包会在选择页检查、下载匹配 CEF 后端的新版 ZIP，确认重启后自动替换旧目录。
...
1. 完整解压 ZIP。
2. 运行目录中的 `DSH-Desktop.exe`。
3. 发行包会在选择页检查、下载匹配 WebView2 后端的新版 ZIP，确认重启后自动替换旧目录。
...
`DSH-Desktop.app`，选择“打开”。已安装的发行包可在选择页检查更新；应用只会下载对应 CPU 架构的 `.app`
压缩包，确认重启后自动解压、替换并重新启动。
...
客户端使用 `target_commitish` 作为发布 commit id，仅当它与应用内的构建 commit id 不一致时提示更新。
Pull Request 只运行测试，不发布产物；任何测试或打包失败都会保留上一版 Release。
