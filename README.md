...
如果远端返回登录要求，应用会通过同一个 SSH Host 尝试从常见远端日志来源恢复启动 token；只有候选 token
能通过当前隧道访问 DSH Web 时才会保存并继续连接。自动恢复失败时，界面会提示输入新的
token。应用使用密钥或 `ssh-agent`，不提供 SSH 密码和私钥 passphrase 的交互输入。

SSH 意外断开（例如系统休眠后连接失效）时，应用返回选择页并显示自动重连进度，依次等待
`1、2、4、8、15` 秒，最多尝试 5 次。可点击“取消重连”或“立即重试”；切换连接或退出应用会取消后台重试。
认证、主机密钥或 token 错误需要人工处理时停止重试。恢复成功后重新打开 DSH
Web；选择页切换和重新加载沿用现有行为，不保留远端页面中未提交的输入。
