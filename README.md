...
应用使用密钥或 `ssh-agent`，不提供 SSH 密码和私钥 passphrase 的交互输入。

**SSH 层只监管进程。** 应用启动专属的前台 SSH 进程，由 OpenSSH 自己处理传输和保活。
应用只在进程非主动退出后等待 1 秒，再创建一个替代进程；这段延迟用于避免进程快速退出时形成忙循环。
用户切换目标、取消重建或退出应用属于主动停止，不再自动创建。SSH 错误信息仅用于诊断，不参与 DSH
判断。 为确保应用监管的就是实际隧道进程，主 SSH 固定使用前台、非 multiplex 模式；不修改用户的 SSH
配置文件。

**DSH 层单独处理 HTTP 和 token。** DSH 检查通过已有隧道访问 Web；收到 `401` 时尝试恢复
token，失败则提示更新。 HTTP 错误、服务不可用、token 失效或取消 DSH 检查，都不会停止或重新创建主 SSH
进程。 “重新检查 DSH”和更新 token 后的检查使用现有进程；等待 DSH 登录期间，SSH
进程若真正退出，仍会按进程规则重建。 DSH 检查成功后打开页面。因 SSH
进程退出而返回选择页、再重新加载页面时，不保留远端页面中未提交的输入。
