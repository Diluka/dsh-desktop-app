...
包名、后端区别、macOS 首次启动方式和更新规则见[发布制品说明](docs/RELEASE_ARTIFACTS.md)。
...
随后在 DSH Desktop 中添加 `my-dsh`，填写远端 DSH Web 端口（默认 `3080`）并连接。应用使用密钥或
`ssh-agent`，不提供 SSH 密码和私钥 passphrase 的交互输入。
